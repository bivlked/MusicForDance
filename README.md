...
- **Без потерь** *(lossless: `WAV`, `FLAC`, `ALAC`)* — звук сохраняется точно бит-в-бит, при повторных операциях ничего не теряется. Сохраняется разрядность *(bit depth)* — 16-, 24- или 32-битная.
- **С потерями** *(lossy: `MP3`, `AAC`, `Opus`, `Vorbis`, `AC3`)* — для уменьшения размера файла часть звуковой информации отбрасывается. Сохраняется тот же битрейт, что у источника.
...
> 📚 **Подробности про lossless / lossy на уровне битов, точность 24-bit `FLAC`, поведение для VBR-кодеков** — [docs/ARCHITECTURE.md §2](docs/ARCHITECTURE.md#2-качество-и-пережатие).
