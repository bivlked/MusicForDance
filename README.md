...
```
input.<any>                                  intro.wav (PCM, генерится в RAM)
     │                                              │
     ▼                                              ▼
silenceremove (detection=peak, window=0,      [aformat fltp/SR/layout]
 sample-accurate trim)                              │
     │                                              ▼
[rubberband tempo=X pitch=1.0]              [asetpts=N/SR/TB]
     │                                              │
     ▼                                              ▼
[aformat fltp/SR/layout]              ┌─────► concat ─────►  output.<src-ext>
     │                                │                       или WAV/MP3 если override
     └────► [asetpts=N/SR/TB] ────────┘
```

Единый процесс ffmpeg на каждую версию, без промежуточных файлов на диске.

---

## Качество и пережатие

**Один цикл декод/процессинг/энкод** — это неизбежно, потому что мы
модифицируем аудио (обрезаем тишину, добавляем тики, замедляем).

### Auto-mode (default)

Тот же codec и качество, что у источника:
- **Lossless** (WAV/FLAC/ALAC): bit depth сохраняется, никаких новых потерь.
- **Lossy** (MP3/AAC/Opus/Vorbis/AC3): re-encoding в тот же codec при тех же
  параметрах. Это всё ещё ОДНА стадия энкода (а не lossy → lossless → lossy
  каскад). На 192+ kbps повторный re-encode практически неотличим от оригинала.

### `--wav`

Принудительный WAV PCM:
- **16/24-bit lossless источники** → WAV того же bit depth, **lossless**.
- **32-bit integer PCM** → пренебрежимая потеря: float32 даёт ~150 dB
  precision против ~192 dB у 32-bit int. Разница ниже порога слышимости.
- **Lossy источники** → WAV 24-bit. Сохраняется ВСЯ информация из лосси-
  источника, без новых потерь сверх существующих в исходнике.

### `--mp3`

ОДНА стадия энкода в MP3 (CBR через libmp3lame). Sample rate автоматически
снапится к ближайшему MP3-rate (8/11.025/12/16/22.05/24/32/44.1/48 кГц).
...
- **Короткий тик** — 60 мс щелчок (1000 Гц + гармоника 2000 Гц с
  экспоненциальным затуханием τ ≈ 15 мс), затем 940 мс тишины.
- **Длинный финальный тик** — ровно 1.0 с (800 Гц + октава 1600 Гц), fade-in
  20 мс / fade-out 100 мс. Заканчивается на самом последнем сэмпле intro —
  следом sample-accurate начинается музыка.

При желании поменять тембр — отредактируй функции `generateTickMono` и
`generateLongBuzzMono` внутри `index.js`.
...
Используется `librubberband` через ffmpeg-фильтр
`rubberband=tempo=X:pitch=1.0` — фазовый вокодер с preservation транзиентов.
На ratio 0.7–0.9× артефакты минимальны для большинства жанров популярной/
танцевальной музыки.

Если хочется максимальное качество (R3 «Finer» engine из librubberband,
недоступный через ffmpeg-фильтр) — установить отдельный
[`rubberband.exe`](https://breakfastquay.com/rubberband/) и переписать
`buildOutput()` с двухпроходным pipeline (decode → rubberband CLI с `--fine`
→ concat). Сейчас этого нет, потому что R2-engine даёт качество, достаточное
для танцкласса.
